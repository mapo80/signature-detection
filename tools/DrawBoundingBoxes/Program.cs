...
string outputDir = Path.Combine(Root, "samples");
...
foreach (var file in files)
{
    ...
    using var predPaint = new SKPaint { Color = SKColors.Lime, Style = SKPaintStyle.Stroke, StrokeWidth = 3 };
    ...
    if (detections.Length > 0)
    {
        ...
        canvas.DrawRect(SKRect.Create(best[0], best[1], best[2] - best[0], best[3] - best[1]), predPaint);
        canvas.DrawText($"Pred {best[4]:0.00}", best[0], Math.Max(0, best[1] - 25), textPaint);
    }
